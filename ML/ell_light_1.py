...
class Ellipse:
    ...
    def parallel_cut(self, g, beta, beta_hat, get_infor=True):
        ...
        print(f"366 ell_light_1: g_norm={g_norm.shape} g_bar={g_bar.shape} g_mat={g_mat.shape}")
        print(f'366 ell_light_1: alpha={alpha} alpha_hat={alpha_hat}')
    ...
    def grid_cut(self, atoms, lamb, nb=10):
        n= atoms.shape[1]
        ids = (n//nb)*torch.arange(0, nb-1) 
        ell=self.copy() 
        for i in range(len(ids)):
            k = ids[i]
            ell, _ = ell.radius_compression_cut(atoms[:, [k]], lamb) 
